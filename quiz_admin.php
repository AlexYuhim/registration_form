...
<h3 class=" text-center mt-5 mb-5">Ответы Анкет</h3>
<div class="cover-container d-flex w-100 h-100 p-3 mx-auto flex-column">
  <form action="apps/plugins/quiz/quiz_ajax.php" class="was-admin" method="get">
...
<script>
  ...
  $("#clear_data").click(function (evt) {
    if (evt) {
      fetch("apps/plugins/quiz/quiz_ajax.php?type=delete")
  ...
  $(".delete_field").click(function (evt) {
    ...
    $('.yes_confirm').click((evt)=>{
      if (evt) {
      fetch("apps/plugins/quiz/quiz_ajax.php?id="+forId+"&date="+forDate)
      .then(
...
// удаляем блок с фио
  $(".delete_blank").click(function (evt) {
    ...
    $('.yes_confirm').click((evt)=>{
      if (evt) {
        fetch("apps/plugins/quiz/quiz_ajax.php?date="+forDate)
        .then(
          response => response.text())
        .then(commits => {
        location.reload();
